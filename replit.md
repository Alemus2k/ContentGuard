...
### Security Features
- **Input Validation & Sanitization** (`modules/input_validator.py`):
  - Text input validation (length limits, script tag detection, unsafe URI scheme detection)
  - HTML escaping of all user text inputs
  - File upload validation (size limits, type checking, magic byte verification for images)
  - Filename sanitization to prevent path traversal
  - Batch input validation with per-line checking
  - Control character stripping
- **Rate Limiting** (`modules/rate_limiter.py`):
  - Session-based rate limiting (20 requests per 60 seconds per analysis type)
  - Separate rate limit tracking for text, image, video, and batch analysis
  - Automatic window expiration and request cleanup
  - User-friendly feedback showing remaining requests and reset time
